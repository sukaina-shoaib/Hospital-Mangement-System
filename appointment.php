...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["doctor_id"])) {
    ...
    $appointment_date = $_POST["appointment_day"]; // The selected date
    ...
    $appointment_query = "INSERT INTO Appointments (token_no, receipt_no, DoctorID, PatientID, time, payment, appointment_date) 
                          VALUES ('$token_no', '$receipt_no', '$doctor_id', '$patient_id', '$appointment_time', '$payment', '$appointment_date')";

    if (mysqli_query($conn, $appointment_query)) {
        // Fetch the inserted appointment details
        $fetch_query = "SELECT token_no, receipt_no, appointment_date,  time FROM Appointments 
                        WHERE PatientID = '$patient_id' AND DoctorID = '$doctor_id' ORDER BY appointment_date DESC LIMIT 1";
...
<head>
    ...
    <style>
        .form-wrapper, .appointment-details { 
            max-width: 600px; 
            margin: auto; 
            padding: 20px; 
        ...
        input{ width: 96%; padding: 10px; margin: 10px 0; }
        button { background-color: #4CAF50; width: 100%;
    ...
    if (!validDaysArray.includes(selectedDay)) {
        ...
        // Populate the hidden field with the selected day
        document.getElementById('appointment_day_hidden').value = selectedDay;
    }
}
...
<?php if ($appointment_details): ?>
    <div class="appointment-details">
        ...
        <div class="details"><strong>Appointment Date:</strong> <?php echo $appointment_details['appointment_date']; ?></div>
