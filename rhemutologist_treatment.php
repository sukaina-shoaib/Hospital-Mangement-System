...
if (isset($_POST['submit'])) {
    ...
    $tests = isset($_POST['tests']) ? $_POST['tests'] : []; // Get test LabIDs
...
$medicines = is_array($medicines) ? implode(', ', $medicines) : $medicines;
$dosages = is_array($dosages) ? implode(', ', $dosages) : $dosages;
$instructions = is_array($instructions) ? implode(', ', $instructions) : $instructions;
$tests = is_array($tests) ? implode(', ', $tests) : $tests;
$test_instructions = is_array($test_instructions) ? implode(', ', $test_instructions) : $test_instructions;
...
$complete_history = $existing_history ? $existing_history . " | " . $new_history : $new_history;

    // Fetch test names based on selected LabIDs
   // Ensure $tests is an array
$tests = is_string($tests) ? explode(',', $tests) : $tests;
$tests = !empty($tests) ? $tests : [];

$test_names = [];
foreach ($tests as $test_id) {
    $test_name_query = "SELECT Name FROM lab WHERE LabID = '$test_id'";
    $test_name_result = mysqli_query($conn, $test_name_query);
    if ($test_name_result && mysqli_num_rows($test_name_result) > 0) {
        $test_name_row = mysqli_fetch_assoc($test_name_result);
        $test_names[] = $test_name_row['Name'];
    }
}

$insert_query = "
    INSERT INTO rheumatologist (doctorid, patientid, disease, history, symptom,  followup_date, medicine, dosage, m_instruction, test, t_instructions) 
    VALUES ('$doctor_id', '$patient_id', '$diseases', '$complete_history', '$symptoms',  '$followup_date', '$medicines', '$dosages', '$instructions', '$test_names_string', '$test_instructions')";

if (mysqli_query($conn, $insert_query)) {
    // Display a success alert using JavaScript
    echo "<script>
            alert('Treatment saved successfully!');
            window.location.href = 'rhemutologist_treatment.php'; // Optional: Redirect to another page
          </script>";
} else {
...
<body>
    <h2>Treatment  <?php echo htmlspecialchars($doctor_name); ?></h2>
        ...
        <tbody id="test-grid">
            <tr>
                ...
                <td>
                    ...
                    <input type="hidden" name="test_instructions[]" class="test-instruction-input" value="">
                </td>
            </tr>
        </tbody>
    </table>
...
<script>
    ...
    function addTestRow() {
        ...
        row.find('.test-instruction-input').val(''); // Clear hidden input value
    ...
    $(document).on('change', '#patient_id', function () {
        ...
        if (patientId) {
            fetch(`?patient=1&PatientID=${patientId}`)
        ...
        if (testId) {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    row.find('.test-instruction-input').val(data.instructions); // Set hidden input value
                }
            });
        } else {
            row.find('.test-instructions').text('');
